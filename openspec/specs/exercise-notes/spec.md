# exercise-notes Specification

## Purpose
Attach a durable **free-text note** to an exercise **within a gym** — one note per
`(gymId, exerciseId)`, like a target weight. Notes capture cues and reminders
("manter cotovelo fixo", "usar pegada aberta") that persist across workout
sessions and are shared between the in-session entry detail and the catalog
exercise detail for that gym.
...
and holds the note **text** and a last-updated timestamp. The note MUST be
**durable**: it is independent of any workout session and persists until edited or
cleared. The **active gym** determines which note is shown and edited. Notes MUST
be **isolated per gym** — the same exercise may carry different notes (or none) in
different gyms.
