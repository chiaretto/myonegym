...
## Purpose
A **ferramenta de manutenção do catálogo oficial**: a tela em `/admin` e a API
que a atende, com as quais se editam as categorias e os exercícios de
`src/data/officialCatalog.json`.

Ela existe porque a alternativa é editar um JSON à mão, e o que se erra ali se
erra **em silêncio**: um `categoryIds` com um número inexistente não quebra
nada, o exercício apenas aparece sem aquela categoria; uma alternativa
declarada de um lado só aparece numa tela e não na outra; e a imagem é um
segundo passo separado, que é justamente o passo que se esquece.

É uma **ferramenta**, não um recurso do app, e isso é o que autoriza tudo o que
ela faz diferente: existe apenas em desenvolvimento, responde apenas ao próprio
computador, tem layout de tela grande num app mobile-first, fica fora do
backup e fora do bundle. Ela edita o **repositório**, não os dados de ninguém —
o desfazer dela é o git.
