...
- [ ] 1.1 `npm i @fontsource/poppins` and
      `npm rm @fontsource/sora @fontsource/manrope @fontsource/jetbrains-mono`.
- [ ] 1.2 Rewrite `src/styles/fonts.css`: it is currently 10 `@import`s of latin
...
- [ ] 2.3 Verify `--fs-xl` is still ≥16px effective at 125% (`1rem × 1.25 = 20px`)
      so iOS does not zoom on input focus.
- [ ] 2.4 **Check the upgrade path for the scale default.** Read
...
- [ ] 3.1 Replace `src/styles/global.css` with `new-design/css/global.css`.
...
- [ ] 3.2 Add `src/styles/icons.css` from `new-design/css/icons.css` and import it
      from `src/main.tsx` next to the Tabler import.
- [ ] 3.3 Replace the four feature sheets from their `new-design/css/`
...
- [ ] 3.4 Keep the cross-file couplings that exist today: `.day-start` is defined
...
- [ ] 3.5 Do **not** introduce `.ex-title` — `detail-header.integration.test.tsx:62`
      asserts it does not exist.
- [ ] 3.6 Do not rename the class names that tests assert on: `screen`,
...
- [ ] 4.1 **Optimise `logo-mark.png` before it ships** — 201 KB today, 85% of the
      ...
      worker precache.
- [ ] 4.2 Copy the 16 brand PNGs to a served location and point `icons.css` at it.
      **Do not copy `new-design/reference/`** — 4.4 MB of design references that
      would be precached. Add it to `.gitignore`.
- [ ] 4.3 Wire the PNG mask glyphs only where the artwork exists: tab bar (home /
...
- [ ] 4.4 Add the muscle-avatar map: a pure `categoria → asset` function with a
...
- [ ] 4.6 Update `vite.config.ts:22-23` `theme_color` / `background_color` and the
      manifest `icons` array (both entries currently point at the same `icon.svg`).
- [ ] 4.7 Update the hardcoded `theme-color` meta at `index.html:7`.

## Phase 5: The canvas share card

- [ ] 5.1 Sync the 11 colours in the `C` block of
      `src/features/session/share/renderCard.ts:15-27` to the new tokens.
- [ ] 5.2 Fix the **12th colour that escaped the block**: an inline
      `rgba(255, 255, 255, 0.14)` at `renderCard.ts:158`.
- [ ] 5.3 Update the canvas font families at `renderCard.ts:29-35` (they name
      ...
      (`renderCard.ts:3-14`).
- [ ] 5.4 Review the baked-in radii (`12`, `8`, `16`, `999`) against the new
      `--r-*` values.
- [ ] 5.5 `src/features/session/share/shareCard.test.ts` must still pass.

## Phase 6: Home — weekly summary (behaviour change)

- [ ] 6.1 Add a helper that buckets the week's completed sessions by weekday,
...
- [ ] 6.2 Replace `WeeklySummary` (`HomePage.tsx:23-53`) with the seven-day track.
      ...
      go together.
- [ ] 6.3 Change `total` from `days.length` to `7` at `HomePage.tsx:132`.
- [ ] 6.4 Add the streak value (consecutive days trained), derived from
      `completedAt`. If the streak is dropped from scope, remove the element —
      nothing else depends on it.
- [ ] 6.5 Cover the zero state: no sessions this week must render "0 / 7" with
...
- [ ] 7.1 Restructure `.day-head` into two lines: the name alone on the first,
      then avatar + categories + `Iniciar` + chevron grouped in `.day-actions`.
- [ ] 7.2 Keep `ul.accordion > li.day` — `next-workout.integration.test.tsx:55`
      does `getByText('Próximo treino').closest('li')`.
- [ ] 7.3 Keep `.day-title` **inside** the toggling button —
      `day-url.integration.test.tsx:74` clicks the day name to expand.
- [ ] 7.4 Keep the start button's accessible name exactly `Iniciar` / `Continuar`
      (8 asserções). Mark the play glyph `aria-hidden` so it contributes no text.
- [ ] 7.5 Keep the exercise row an `<a>` with `href="/exercise/<id>?day=<dayId>"`
      (`day-url.integration.test.tsx:141`).

## Phase 8: Remove the inline styles that beat the new CSS

- [ ] 8.1 `src/features/gym/GymSelector.tsx:21` — inline `padding` and
      `fontSize: 12` on the `.chip.accent` pill override the redesigned chip.
- [ ] 8.2 `src/ui/Chrome.tsx:30` — inline `fontSize`/`fontWeight` on the BackBar
      `<h1>` overrides `.appbar h1`.
- [ ] 8.3 `src/features/settings/SettingsPage.tsx:32` — inline `fontSize: 16` on
...
- [ ] 9.1 `npm test` — all 38 test files, **no asserção alterada**. If a test
      fails, fix the code, not the test.
- [ ] 9.2 **Watch `App.test.tsx:29` specifically.** It does
      ...
      not the assertion.
- [ ] 9.3 `./node_modules/.bin/tsc -p tsconfig.json --noEmit` clean.
- [ ] 9.4 `npm run build` passes, and the precache did not gain a reference image.
