...
**Affects:** conta Google (login por redirecionamento), backup e restauração
pelo Google Drive (`appDataFolder`), tela de Conta, grupo "Google Drive" na
tela de Backup

Capability nova. Purpose sugerido para o arquivo de spec ao arquivar:

> Uma conta Google conectada ao app, e uma cópia do backup completo guardada
> na pasta oculta (`appDataFolder`) do Drive do próprio usuário. Com a mesma
> conta em dois aparelhos, os dois enxergam a mesma cópia. Nada acontece sem
> um toque: o app fala com o Google apenas ao entrar, sair, fazer backup ou
> restaurar.
