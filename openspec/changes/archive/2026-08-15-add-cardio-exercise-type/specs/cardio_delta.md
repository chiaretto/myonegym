# Delta: cardio (capability nova)

**Change ID:** `add-cardio-exercise-type`
**Affects:** `src/ui/Chrome.tsx` (`TabBar`), `src/App.tsx` (rota `/cardio`),
`src/features/cardio/CardioPage.tsx` (novo), `src/db/repos.ts`
(`startCardioSession`, `listCardioExercises`)
