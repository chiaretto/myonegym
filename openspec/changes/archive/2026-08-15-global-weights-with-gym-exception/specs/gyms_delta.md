...
**Affects:** `src/db/repos.ts` (`createGym`, `deleteGym`),
`src/features/settings/GymsPage.tsx`
...
*(requisito **removido** — ver REMOVED abaixo)*
...
O formulário de criação de academia deixa de oferecer "Copiar pesos de
(opcional)", e `createGym` deixa de aceitar uma academia de origem.
